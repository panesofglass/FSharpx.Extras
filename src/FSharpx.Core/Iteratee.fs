...
module Iteratee =
    ...
    type Iteratee<'el,'a> = Stream<'el> -> IterResult<'el,'a>
    and IterResult<'el,'a> =
        | Done of 'a * Stream<'el>
    ...
    let empty<'a> : Iteratee<'a,_> = returnI ()
    let throwI e  : Iteratee<_,_>  = fun s -> Error(e, None, Some s)

    // TODO: re-apply fix
    let rec bind (f:'a -> Iteratee<'b,'c>) (m:Iteratee<'b,'a>) : Iteratee<'b,'c> = fun s ->
        match m s with
        | Done(a,s')    -> (f a) s'
...
module Inum =
    ...
    type Inum<'eli,'elo,'a> = Iteratee<'elo,'a> -> Iteratee<'eli, IterResult<'elo,'a>>

    /// An enumerator generates a stream of data and feeds an iteratee, returning a new iteratee.
    type Onum<'el,'a> = Inum<unit,'el,'a>
