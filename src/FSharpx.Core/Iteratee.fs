...
type Iteratee<'el,'a> =
  Iteratee of (('a -> Stream<'el> -> unit) -> ((Stream<'el> -> Iteratee<'el,'a>) -> exn option -> unit) -> unit)
...
module Iteratee =
  ...
  let bind (m: Iteratee<'el,'a>) (f: 'a -> Iteratee<'el,'b>) : Iteratee<'el,'b> =
    let rec inner m f =
      Iteratee <| fun onDone onCont ->
        let mdone a s =
  ...
  let tryRun finish i =
    let rec onDone x _ = finish <| Choice1Of2 x
    and onCont k = function
      | None -> runIter (k (EOF None)) onDone onCont'
      | Some e -> finish <| Choice2Of2 e
    and onCont' k = function
      | None -> finish <| Choice2Of2 (Exception("divergent iteratee"))
      | Some e -> finish <| Choice2Of2 e
    in runIter i onDone onCont
